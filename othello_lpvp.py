...
form_class_lpvp = uic.loadUiType("ui/dlg_lpvp.ui")[0] #UI 불러오기


class WindowClassLpvp(QMainWindow, form_class_lpvp) : #로컬 PVP 대전 클래스 선언(화면 띄움)
    def __init__(self, parent=None) :
        super(WindowClassLpvp, self).__init__()
        self.setupUi(self)
        self.setWindowTitle("Local Game : Player1 vs Player2")
    ...
    def drawGame(self, painter):
        ...
        if player == '1' and Available(board, p_x, p_y, player): #흑돌일때
            pixmap_turn1 = QPixmap("res/turn_red.png") #턴표시
        ...
        elif player == '2' and Available(board, p_x, p_y, player): #백돌일때
            pixmap_turn2 = QPixmap("res/turn_red.png") #턴표시
            painter.drawPixmap(cell_size*8+cell_size, cell_size*1+cell_size+10, 70, 60,pixmap_turn2)
            pixmap2 = QPixmap("res/mal_white.png")
            painter.drawPixmap(cell_size*p_x+cell_size -10, cell_size*p_y+cell_size + 60, 50, 50,pixmap2)
            board[p_y][p_x] = '2' #돌두기
            (board, total) = Move(board, p_x, p_y, player) #뒤집기
            player = '1' #턴넘기기
            for y in range(8): #저장되있는 돌 그려주기
